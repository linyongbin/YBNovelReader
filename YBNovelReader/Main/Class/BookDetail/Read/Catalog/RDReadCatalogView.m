...
{
    _book = book;
    self.header.nameLabel.text = book.bookName;
...
{
    if (!_tableView) {
        _tableView = [[UITableView alloc] initWithFrame:CGRectMake(0, 50, ScreenWidth, self.contentView.height-50-50-SafeAreaBottomHeight) style:UITableViewStylePlain];
...
-(RDReadCatalogHeader *)header
{
    if (!_header) {
        ...
        _header.backgroundColor = UIColorFromRGB(0xe2e2e2);
    }
    return _header;
}
-(UIView *)contentView
{
    if (!_contentView) {
        _contentView = [[UIView alloc] initWithFrame:CGRectMake(0, ScreenHeight, ScreenWidth, ScreenHeight-SafeAreaTopHeight)];
    }
    return _contentView;
}
...
-(void)show
{
    ...
    self.backgroundColor = [UIColor colorWithWhite:0 alpha:0];
       [UIView animateWithDuration:.35 animations:^{
           ...
           self.contentView.frame = CGRectMake(0, SafeAreaTopHeight, ScreenWidth, self.height-SafeAreaTopHeight);
       }];
}
-(void)dismiss
{
    [UIView animateWithDuration:.3 animations:^{
        self.backgroundColor = [UIColor colorWithWhite:0 alpha:0];
        self.contentView.frame = CGRectMake(0, ScreenHeight, ScreenWidth, self.height-SafeAreaTopHeight);
